...
class ChannelTransferService {
  ...
  Future<bool> createChannelTransfer(Map<String, dynamic> payload) async {
    try {
      ...
      if (success) {
        final bankName = payload['bank_name'] ?? '';
        await _logService.createLog(
          kategori: 'Channel Transfer',
          deskripsi:
              bankName.toString().isNotEmpty
                  ? 'Menambahkan channel transfer bank $bankName'
                  : 'Menambahkan channel transfer baru',
        );
      }
      return success;
    } catch (e) {
    ...
    try {
      ...
      if (success) {
        ...
        await _logService.createLog(
          ...
          deskripsi:
              bankName.toString().isNotEmpty
                  ? 'Memperbarui channel transfer bank $bankName (ID: $id)'
                  : 'Memperbarui channel transfer (ID: $id)',
        );
      }
      return success;
    } catch (e) {
  ...
  Future<bool> deleteChannelTransfer(int id) async {
    try {
      ...
      if (success) {
        await _logService.createLog(
          kategori: 'Channel Transfer',
          deskripsi: 'Menghapus channel transfer (ID: $id)',
        );
      }
      return success;
    } catch (e) {
