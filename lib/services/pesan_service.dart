...
class PesanService {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        await _logService.createLog(
          kategori: 'Pesan Warga',
          deskripsi: 'Mengirim pesan "$judul" kepada user ID $penerimaId',
        );
        return true;
      }
  ...
  Future<bool> kirimAspirasi({
    ...
    try {
      ...
      if (response.statusCode == 201) {
        await _logService.createLog(
          kategori: 'Pesan Warga - Aspirasi',
          deskripsi: 'Mengirim aspirasi "$judul"',
        );
        return true;
      }
      final body = jsonDecode(response.body);
  ...
  Future<AspirasiWarga> updateAspirasi({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        await _logService.createLog(
          kategori: 'Pesan Warga - Aspirasi',
          deskripsi: 'Memperbarui aspirasi (ID: $id) dengan judul "$judul"',
        );
  ...
  Future<AspirasiWarga> updateAspirasiStatus({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        await _logService.createLog(
          kategori: 'Pesan Warga - Aspirasi',
          deskripsi: 'Memperbarui status aspirasi (ID: $id) menjadi $status',
        );
        return AspirasiWarga.fromJson(jsonDecode(response.body));
      }
      final body = jsonDecode(response.body);
  ...
  Future<bool> deleteAspirasi(int id) async {
    try {
      ...
      if (response.statusCode == 200) {
        await _logService.createLog(
          kategori: 'Pesan Warga - Aspirasi',
          deskripsi: 'Menghapus aspirasi (ID: $id)',
        );
        return true;
      }
      final body = jsonDecode(response.body);
